...
async function main() {
    fs.writeFileSync(LOG_FILE, 'Start Bulk Audits V2\n')
    ...
    for (let i = 1; i <= 3; i++) {
        const auditCode = `BULK-FIX-${ts}-${i}`
        const productName = `Product Fix ${ts} ${i}`

        // Create App - OMITTING applied_standard
        const { data: app, error: appErr } = await supabase.from('audit_applications').insert({
            user_id: adminId,
            client_org_id: orgId,
            ...
            // applied_standard: 'ISO 17065', // OMITTED
            content: { applied_standard: 'ISO 17065' },
            created_by: adminId
        }).select().single()
